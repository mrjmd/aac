...
**Status (2026-05-30):** Walk pieces 1–4 (all sub-pieces) shipped. Pipeline now covers all six trigger paths end-to-end through to the shadow queue: QB Estimate.Update webhook → `normalizeQbApproval`; daily QB reconciliation cron backstop; Quo `message.received` → customer classifier → `normalizeQuoCustomerIntent` (with calendar-based callback parent resolution); Quo `message.delivered` → matt classifier → `normalizeManualSchedule`; Quo `call.transcript.completed` → BOTH speakers classified in parallel → either normalizer. Directives carry `durationPrediction` (where attached to QB estimates) and write to `scheduling:pending:list` for command-center review. Walk #4a: `GeminiClient.classifySchedulingIntent` shipped 2026-05-29 in `@aac/api-clients` — 19 tests, separate prompts per speaker role, role-guard parser. Walk #4b: shipped 2026-05-30 — `normalizeQuoCustomerIntent` + `resolveCallbackParent` in `@aac/scheduling` (24 new tests) and `apps/middleware/lib/scheduling-dispatch.ts` wired into `apps/middleware/api/webhooks/quo.ts` (15 new tests). Callback parent resolution: most recent calendar event with `colorId='10'` whose summary matches customer name; `parentDealId` from `[deal:N]` marker; `callbackSequence` = count of subsequent `colorId='5'` events + 1. Per Matt: PD field lookup becomes canonical once deal-state backfill is solid — same single function will swap source. `/api/health` exposes new `schedulingClassifier.byEventType` block (classified + directivesWritten over 48h) so we have real data on transcript volume instead of guesses. **NOT yet deployed to prod — that's the deliberate next step before Walk #5.** Walk #6 (propose-dialogue endpoint) remains the customer-visible payoff. Test counts: scheduling 83 (+24), middleware 188 (+15), api-clients 234.
**Owner:** Matt
...
| 4b | Quo-path normalizers (`normalizeQuoCustomerIntent` + `resolveCallbackParent`) + middleware dispatch helper + Quo webhook wire-up + /health classifier counters | `apps/middleware` + `@aac/scheduling` | ✅ shipped 2026-05-30 (54 new tests; not yet deployed) |
