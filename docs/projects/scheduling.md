...
**Status (2026-05-30):** Walks 1–6 shipped + Walk #6 smoke-test executed end-to-end (Margie Mercadante directive, dir `5390ea40…`) + **Walk #6.5 shipped + two follow-up fixes + deployed to prod end-to-end-validated** (latest mw `aac-middleware-monorepo-5uzo6jkqq`, deployed 2026-05-30 with new `GOOGLE_MAPS_API_KEY` env reused from apps/field). The Walk #6.5 deploy pipeline executed three deploys:

1. `aac-middleware-monorepo-3aprt84pq` — Walk #6.5 commit `1ac568b` (algorithm + confidence gate + travel primitive). Margie re-fire returned v0 reasoning because her Quo-text directive carried no `qbCustomerId`.
2. `aac-middleware-monorepo-l24fnq3mi` — Follow-up commit `a931a01`: `apps/middleware/lib/scheduling-dispatch.ts:fetchPdPersonInfo` now resolves `qbCustomerId` from PD person's QB cross-system custom field (Redis-cached via existing PD→QB map), passes it onto the directive, so `proposal-builder.fetchCustomer` can pull QB BillAddr for the customer address. Margie's existing directive was patched in Redis directly (set `qbCustomerId: "1400"`) to validate the path. Re-fire returned travel-aware reasoning… but the proposed slot **overlapped** Sean's 8:30–16:30 job on Mon Jun 1 because the gap walker treated overlapping events as separate fences.
3. `aac-middleware-monorepo-5uzo6jkqq` — Hotfix commit `0cc4fbf`: `findSlotOnDayWithTravel` now merges overlapping/nested events into busy spans before fencing. Each span tracks `departLocation` (latest-finishing event's location, used as origin for "drive INTO customer") and `arriveLocation` (earliest-starting event's location, used as destination for "drive OUT of customer"). Regression test asserts Mon Jun 1 is skipped when Sean + nested Ed-1on1 are present.

Final Margie smoke-test slot (proposal `prop_414468e1d7fa40de`): **Mon Jun 8 11:33am ET, 2h on-site** with reasoning `previous job → customer 49min, 2h on-site, customer → home (back by deadline) 71min (+15min buffer each leg)`. This matches Matt's natural pick during the Walk #6 debrief ("my natural inclination would have been to put it on Monday after the Pembroke job"). Mon Jun 1 + Tue Jun 2 are both correctly rejected (Sean fills both days' usable windows once drive-time to Falmouth is accounted for).

**Test counts post-#6.5+follow-ups:** scheduling 105 (+7 travel-aware including overlap regression), middleware 218 (+3 qbCustomerId resolution), agent 102, api-clients 234. All green.
...
- **Pre-Walk-#6.5 directives still missing `qbCustomerId`.** The dispatch fix (`a931a01`) only takes effect on NEW directives. Any directive written before that commit (e.g., Margie's was written 2026-05-30T12:29 — before the deploy) is missing the field. Options: (a) tolerate the gap and let the queue drain naturally as new directives arrive, (b) run a one-shot backfill cron that walks `scheduling:pending:list`, looks up each directive's `pdPersonId` in PD, sets `qbCustomerId` if found. Current decision: tolerate.
- **PD-address-only fallback when QB customer is absent.** Even with the dispatch fix, a PD person who's not yet linked to QB (`a02e76a3d2d7e38cacd476aaea1c2a8809264025` field empty) leaves `customerAddress` null. PD does carry a custom address field (hashed key `5fc7cf5d8c890fe2f7062aaabe1e9b416c851511` with `_route`/`_locality`/`_admin_area_level_1`/`_postal_code` siblings). Wire that as a third tier in `proposal-builder.fetchCustomer` so `assessment_requested` paths on QB-less customers still get travel-aware slots. Not blocking — `quote_approved` directives always carry QB via dispatch, and assessments default to v0 which is acceptable for now.
