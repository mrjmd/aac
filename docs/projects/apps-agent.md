...
5. ⏳ **Nightly deal-reconcile cron** — reconciles QB estimate/invoice state into deal stages: open estimate without a deal → create at Quote Sent; estimate accepted → Quote Accepted; invoice paid → Paid. Catches anything not stamped by deterministic write paths. *Next.*
