...
1. ✅ **Agent comms inbound handler** — *Shipped 2026-05-28.* `apps/agent/api/webhooks/quo.ts` (Web Standard API `export POST`, same pattern as middleware's quo webhook) receives Quo events on the agent line, verifies HMAC via `@aac/shared-utils/webhook-signature` (extracted from middleware in the same change; middleware migrated to import it — DRY), dedups via `keys.dedupe('quo-agent', eventId)` (separate namespace from middleware's `quo` source), parses, then dispatches to `lib/inbound-handler.ts`. The core handler filters to `message.received` only (call.* + message.delivered audited as `unsupported_event`), confirms the message landed on the agent comms line (defensive `wrong_line` check), looks up the sender via `lookupRole` against `AGENT_USER_ROLES`, and routes through `lib/intent-router.ts`. **Stub router policy:** owner → ack with "intent classification isn't live yet"; technician/salesperson/triage → ignore (placeholder roles per spec); unknown sender → silent drop + `unknown_caller` audit (no replies to non-mapped numbers — customers must never get a bot reply). Reply is sent via Quo `sendMessage` from the agent line. Every path writes an `AgentAuditEntry` to `agent:audit:stream` (capped LIST, 1000 entries — replaces the originally-planned stream semantics; same shape as `health:errors`). Webhook returns 200 on internal failures so OpenPhone doesn't retry-double-process; non-200 only on bad sig (401), bad payload (400), or missing `QUO_WEBHOOK_SECRET` (503). 24 new tests (8 sig + 4 intent + 6 dedup/audit + 9 inbound-handler + 7 HTTP wrapper); full monorepo green. **Activation completed 2026-05-28** (live in prod): (a) OpenPhone webhook subscription created on the agent line `(617) 766-0151` pointing at `https://agent-matt-davis-projects-40d1f910.vercel.app/api/webhooks/quo` (events: `message.received`); (b) per-URL signing secret installed as `QUO_WEBHOOK_SECRET` on the agent Vercel project (NOT shared with middleware — OpenPhone signs each webhook URL with its own secret); (c) `AGENT_USER_ROLES={"+16176681677":"owner"}` set so Matt texting the agent line from the OpenPhone app on the business line is identified as owner; (d) Vercel SSO (`ssoProtection`) disabled on the agent project so OpenPhone POSTs reach the endpoint — matches middleware setup; sensitive surfaces remain protected by their own HMAC / `CRON_SECRET` auth. End-to-end smoke test passed: Matt texted the agent line and got the stub ack reply. **Bifurcation TODO:** today only Matt texts from `+16176681677`; once anyone else (future inbound triage hire) starts answering the phone, this single mapping no longer distinguishes per-purpose and we'll need to either (i) assign a separate OpenPhone "from" number per person, or (ii) move identity from sender phone to something stronger (PIN, signed link, OpenPhone user ID if exposed via webhook). **LLM choice for Walk 3 onward (intent classification + nudges + diagnostic agent) is not yet locked** — see `apps/agent/CLAUDE.md` env table for the wiring expectation; tradeoff is fan-out (Gemini Flash for high-volume per-message classification, Claude Sonnet/Opus for low-volume reasoning calls like full-history nudge drafting and error diagnosis) vs single-model simplicity. Decide right before Walk 3.
2. ✅ **Agent read-access tool surface** — *Shipped 2026-05-28.* Seven LLM-callable functions for cross-system queries, each a pure function under `apps/agent/lib/tools/`:
   - `getCustomerContext({ personId? | phone?, recentDays? })` — person + all deals + recent Quo activity + recent calendar events tagged with their `[deal:N]` markers
   - `searchCalendar({ rangeStart, rangeEnd, locationKeyword?, color? })` — date-range required; color maps `job→10`, `assessment→3`, `any→no filter`
   - `listDeals({ stage?, personId?, rangeStart?, rangeEnd?, limit? })` — uses `getDealsByPerson` when `personId` given, else `pd.listDeals` (new client method)
   - `getDeal({ dealId, eventLookback{Days|ForwardDays}? })` — deal + person + QB estimate + QB invoice + events with marker
   - `findJobsMissingInvoices({ rangeStart, rangeEnd })` — green-only events with reason `no_deal_link` / `deal_has_no_invoice` / `invoice_not_found_in_qb`
   - `getInvoiceSummary({ rangeStart?, rangeEnd? })` — defaults to current UTC month; totals + paid/unpaid + per-customer
   - `searchConversation({ personId? | phone?, query?, limit? })` — primary-phone fallback when personId given; case-insensitive substring filter
   
   **Architecture:** every tool is `(deps, [config], input) → Promise<Summary>` — never throws on "not found" (returns null/empty), always throws on transport errors. Summary shapes (`PersonSummary`, `DealSummary`, etc.) are compact LLM-friendly projections in `tools/types.ts` with mapper helpers from raw client types. **Per-role registry** in `lib/tools/index.ts` exposes the seven tools to `owner` (Matt) and empty arrays to `technician`/`salesperson`/`triage` (placeholders per spec). Each entry carries `name`, `description`, JSON-schema-shaped `inputSchema`, and an `invoke(args)` with deps + config curried — ready for Anthropic tool-use / Gemini function-calling at Walk 3. **Shared work:** `parseDealMarker` moved from `apps/middleware/lib/cron.ts` to `@aac/api-clients/pipedrive` (DRY — both apps need it; middleware migrated to import from shared in the same change). New `PipedriveClient.listDeals({stage?, status?, limit?})` method. New agent client factories `getQuickBooks()` + `getCalendar()` in `apps/agent/lib/clients.ts`; QB token store helpers in agent `redis.ts` (same `keys.qbOAuthTokens` slot as middleware). 48 new agent tests (9 customer-context + 6 search-calendar + 5 list-deals + 4 get-deal + 6 missing-invoices + 5 invoice-summary + 7 search-conversation + 6 registry); 146 middleware + 214 api-clients + 100 agent tests green; full monorepo typecheck green.
