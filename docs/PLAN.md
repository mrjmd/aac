...
| 2 | **apps/agent** | Agent platform (comms line, deal spine, read-tool surface, intent classification) | **Crawl phase complete (steps 1–8) 2026-05-28.** Shared: PD Deal CRUD + Quo conversation methods in `@aac/api-clients`. Middleware: `[deal:N]` marker support, inbound-lead deal stamp on Quo webhook, nightly QB→PD deal-reconcile cron. Tools: one-shot backfill script (production `--apply` still gated on Funnel A Phase 1 cleanup). Apps/agent: new pillar scaffolded with env/roles/clients/redis libs, `/api/health` endpoint, and error-surface cron texting Matt middleware errors from the agent comms line every 10 minutes. Walk phase next: agent comms inbound handler + read-tool surface + intent classification + Funnel A Phase 2 stale-deal nudges. | `projects/apps-agent.md` |
