...
**Last updated:** 2026-05-29 (Walk pieces #2 + #3 shipped: `suggestSlot` v0 + daily QB reconciliation cron. Pipeline is now: webhook → directive OR cron backstop → directive → slot suggestion. Next: Walk #4 = Gemini classifier extension to unblock Quo-text trigger paths; Walk #6 = propose-dialogue is the customer-visible payoff)
