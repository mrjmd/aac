...
**Last updated:** 2026-05-30 (Walks #2 + #3 + #4 deployed `dpl_EZfkctJz8gszP2LRkZM7DraDmWpf`; Walk #5 `buildEventDescription` package-only (first consumer is Walk #6); Walk #6 propose-dialogue shipped + both apps deployed 2026-05-30 + smoke-tested end-to-end with Margie's directive (mw `aac-middleware-monorepo-2k46niw6p`, ag `agent-a9c6bqjd5` after the `\n`-in-env infra patch). Smoke-test surfaced three orthogonal algorithm + dispatch gaps that promote **Walk #6.5** ahead of Walk #7: (1) `defaultAssessmentHours: 1 → 0.5`, (2) travel-time feasibility + 15-min buffer + 5:30 PM return-home anchor at 30 Randlett Street Quincy, reusing `@aac/api-clients/google-maps` already wired in apps/field, (3) confidence gate at dispatch (directives with `confidence.score < 0.7` route to `scheduling:pending-review:list` instead of auto-proposing). Classifier semantic surgery (context-aware intent + `appointment_confirmed` + slot extraction) is documented as a separate deferred spike — confidence gate is the safety net that lets it wait. Pipeline covers all six trigger paths through to the shadow queue. 4 known shadow-tolerable dedup/cleanup gaps documented as Walk-#7 prerequisites in `docs/projects/scheduling.md`. Next: **Walk #6.5** (~half day), then Walk #7 (`executeDirective`) — must clear the four prereqs first.)
