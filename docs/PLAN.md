...
**Last updated:** 2026-05-29 (Crawl complete: QB webhook verified e2e in prod; duration-analysis spike done; classifier refined with warranty stripper; reference dataset ready for `@aac/quoting` codification)
...
| 2 | **`@aac/scheduling` pipeline + middleware webhook integration** | SchedulingDirective pipeline (slot suggestion + event creation + PD deal updates + callback child-deal logic) called from middleware Quo webhook (intent extraction) + new QB Estimate webhook + daily QB reconciliation backstop cron | **Active — Crawl complete; Walk codification next.** ✅ SchedulingDirective type + 2 normalizers + 42 tests in `@aac/scheduling`. ✅ `/api/qb-webhook` live in prod, e2e-verified 2026-05-29 (real Estimate flip → directive at confidence 1.0). ✅ Backtest harness scaffold (QB path live; manual-schedule stub). ✅ **Duration analysis spike complete** (`tools/src/scratch/spike-duration-analysis.ts`): 180d, n=54 reliable pairs, classifier with warranty stripper, 2-category taxonomy per Matt, Dillon-pattern data-quality filter. Crack injection earns 1.37× more per crew-hour than concrete resurfacing. Reference dataset at `spike-output/duration-analysis-2026-05-29.json`. **Next:** codify `@aac/quoting/estimate-duration` per `projects/duration-heuristic-design.md`, then Gemini classifier extension, command-center pending-directives view, daily reconciliation cron, Walk phase (slot suggestion + propose-dialogue). | `projects/scheduling.md`, `projects/duration-heuristic-design.md` |
