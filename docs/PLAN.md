...
**Last updated:** 2026-05-30 (Walk pieces #2 + #3 + #4 (all sub-pieces) shipped. Pipeline now covers all six trigger paths end-to-end through to the shadow queue: QB Estimate.Update + daily reconciliation + Quo `message.received` (customer classifier) + Quo `message.delivered` (matt classifier) + Quo `call.transcript.completed` (both speakers classified in parallel). Walk #4b shipped 2026-05-30 — `normalizeQuoCustomerIntent` + `resolveCallbackParent` (calendar Color ID 10 lookup, PD-field migration path noted) + middleware dispatch helper + Quo webhook wire-up + /health classifier counters. NOT yet deployed to prod. Next: deploy + watch the queue + Walk #5 (buildEventDescription) or Walk #6 (propose-dialogue, customer-visible payoff))
...
| 2 | **`@aac/scheduling` pipeline + middleware webhook integration** | SchedulingDirective pipeline (slot suggestion + event creation + PD deal updates + callback child-deal logic) called from middleware Quo webhook (intent extraction) + new QB Estimate webhook + daily QB reconciliation backstop cron | **Active — Crawl complete + duration heuristic shipped + Walk pieces 1–4 shipped (3 deployed; 4b not yet deployed); Walk #5 (buildEventDescription) or Walk #6 (propose-dialogue) next.** ✅ SchedulingDirective type + 4 normalizers + 83 tests in `@aac/scheduling` (includes `normalizeQuoCustomerIntent` + `resolveCallbackParent` shipped 2026-05-30 with 24 new tests). ✅ `/api/qb-webhook` live in prod, e2e-verified 2026-05-29. ✅ Duration heuristic codified (`@aac/quoting/{classify-scope,estimate-duration}` + reference JSON + 34 tests); wired into `normalizeQbApproval` so directives carry full `durationPrediction`. ✅ Command-center `/scheduling` view live. ✅ `@aac/scheduling/suggestSlot` v0 — pure function (16 tests); soft 2 jobs/day with two-pass cap relaxation, no Sat default, 21-day lookahead, 08:00–17:00 ET, DST-correct. ✅ `/api/cron/qb-reconcile` daily 15:00 UTC backstop (8 tests); deployed in `dpl_3qKzhd8ZmfpbmV6zw9rQ5CTdqHa4`. ✅ `GeminiClient.classifySchedulingIntent` shipped in `@aac/api-clients` (19 tests) — role-based prompts, role-guard parser. ✅ Walk #4b (2026-05-30): Quo-path normalizers + dispatch helper + Quo webhook wire-up. Pipeline now covers all six trigger paths through to shadow queue: QB Estimate.Update + daily reconciliation + Quo `message.received` (customer classifier) + Quo `message.delivered` (matt classifier) + Quo `call.transcript.completed` (both speakers in parallel). Callback parent resolution via calendar Color ID 10 lookup + `parseDealMarker`; PD-field migration path noted for once deal-state backfill is solid. New `/api/health.schedulingClassifier.byEventType` exposes 48h volume per event type. Middleware test suite: 188 (+15 from Walk #4b). NOT yet deployed. **Walk build order:** (1) ✅ command-center view, (2) ✅ `suggestSlot` v0, (3) ✅ daily reconciliation cron, (4a) ✅ Gemini classifier method, (4b) ✅ classifier wire-up + Quo-path normalizers (un-deployed), (5) `buildEventDescription`, (6) propose-dialogue endpoint, (7) `executeDirective`. | `projects/scheduling.md`, `projects/duration-heuristic-design.md` |
