...
**Last updated:** 2026-05-29 (Walk pieces #2 + #3 + #4a shipped: `suggestSlot` v0 + daily QB reconciliation cron deployed `dpl_3qKzhd8ZmfpbmV6zw9rQ5CTdqHa4`; `GeminiClient.classifySchedulingIntent` shipped in `@aac/api-clients` with 19 tests (role-based prompts: customer → quote_approved/assessment_requested/callback_opened, matt → manual_schedule with time extraction). Next: Walk #4b = wire classifier into Quo webhook + add Quo-path normalizers; Walk #6 = propose-dialogue is the customer-visible payoff)
...
| 2 | **`@aac/scheduling` pipeline + middleware webhook integration** | SchedulingDirective pipeline (slot suggestion + event creation + PD deal updates + callback child-deal logic) called from middleware Quo webhook (intent extraction) + new QB Estimate webhook + daily QB reconciliation backstop cron | **Active — Crawl complete + duration heuristic shipped + Walk pieces 1–3 deployed + Walk #4a (Gemini classifier method) shipped; Walk #4b (wire into Quo webhook + Quo-path normalizers) next.** ✅ SchedulingDirective type + 2 normalizers + 59 tests in `@aac/scheduling`. ✅ `/api/qb-webhook` live in prod, e2e-verified 2026-05-29. ✅ Backtest harness scaffold. ✅ Duration heuristic codified (`@aac/quoting/{classify-scope,estimate-duration}` + reference JSON + 34 tests); wired into `normalizeQbApproval` so directives carry full `durationPrediction`. ✅ Command-center `/scheduling` view live — reads `scheduling:pending:list`, renders cards with intent + event class + customer + scope + duration prediction + similar cases + confidence chips. ✅ `@aac/scheduling/suggestSlot` v0 — pure function (16 tests); soft 2 jobs/day with two-pass cap relaxation, no Sat default, 21-day lookahead, 08:00–17:00 ET, DST-correct; manual_schedule knownSlot short-circuits; multi-day refused. ✅ `/api/cron/qb-reconcile` daily 15:00 UTC backstop (8 tests); dedup via `scheduling:directive-by-qb-estimate:{id}` reverse index populated by `writePendingDirective`; deployed in `dpl_3qKzhd8ZmfpbmV6zw9rQ5CTdqHa4`. ✅ `GeminiClient.classifySchedulingIntent` shipped in `@aac/api-clients` — focused method (kept separate from `extractEntities` to preserve prompt quality); role-based prompts (customer → 3 intents, matt → manual_schedule with `knownSlot`/`eventClass`/`scopeSummary` extraction); injects today's date + timezone for natural-language date resolution; role-guard drops out-of-range model outputs; 19 tests. **Walk build order:** (1) ✅ command-center view, (2) ✅ `suggestSlot` v0, (3) ✅ daily reconciliation cron, (4a) ✅ Gemini classifier method, (4b) classifier wire-up + Quo-path normalizers, (5) `buildEventDescription`, (6) propose-dialogue endpoint, (7) `executeDirective`. | `projects/scheduling.md`, `projects/duration-heuristic-design.md` |
